...
const Landing = () => {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    if (!data.roomId) {
      alert("The room ID can't be empty");
    } else {
      setLoading(true);
      try {
       const id =  await (await axios.post("http://localhost:3000/CreateRoom", data)).data.newRoom.id;
        reset(); // Clear form 
        navigate(`/room/${id}`); 
      } catch (err) {
        console.error(err);
  ...
  return (
    <div className='w-full text-white overflow-hidden relative h-screen f2 bg-[#0f0f0f]'>
      {/* this is the overlay for the room number form  */}
      {/* <div className='w-full h-full text-black bg-black/70 absolute pointer-events-auto z-[55] top-0 left-0 flex justify-center items-center'>
        <div className='w-[30%] h-[70%] bg-red-600 p-10 rounded-lg'>
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className='flex flex-col gap-1 w-[60%]'>
              <label>Room_ID</label>
              <input {...register("roomId")} type="text" name="roomId" placeholder='enter room Id' />
            </div>
            <div className='mt-2 w-[40%]'>e
              <button
                type="submit"
                className={`w-full h-full bg-white text-black ${loading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-200'} transition duration-300 ease-in-out`}
                disabled={loading}
              >
      ...
      <div onClick={() =>console.log("clicked once")} className='absolute  z-50 cursor-pointer bottom-8 f2 text-sm flex items-center justify-center left-[50%] w-32 h-12 shadow-lg border border-white/10 rounded-full -translate-x-[50%]'>
