...
import { createRoom, FindRoom, updateTextRoomValue } from "../controllers/RoomController";
...
router.post("/enterContent" , (req:Request , res:Response)=>{
               const payload = req.body ; 
               console.log(payload)
               try{
                              const updatedContent = updateTextRoomValue(payload)
                              res.status(200).json({
